...
if ($action === 'get') {
    ...
    $user_reaction = null;
    if (isset($_SESSION['user_id'])) {
        $user_sql = "SELECT loai_cam_xuc FROM cam_xuc_san_pham WHERE vay_id = ? AND nguoi_dung_id = ?";
        $user_stmt = $conn->prepare($user_sql);
        $user_stmt->bind_param("ii", $vay_id, $_SESSION['user_id']);
        $user_stmt->execute();
...
if ($action === 'toggle') {
    ...
    $check_sql = "SELECT loai_cam_xuc FROM cam_xuc_san_pham WHERE vay_id = ? AND nguoi_dung_id = ?";
    $check_stmt = $conn->prepare($check_sql);
    $check_stmt->bind_param("ii", $vay_id, $user_id);
    $check_stmt->execute();
    ...
    if ($check_result->num_rows > 0) {
        ...
        if ($current['loai_cam_xuc'] === $loai_cam_xuc) {
            $delete_sql = "DELETE FROM cam_xuc_san_pham WHERE vay_id = ? AND nguoi_dung_id = ?";
            $delete_stmt = $conn->prepare($delete_sql);
            $delete_stmt->bind_param("ii", $vay_id, $user_id);
            $delete_stmt->execute();
            
            echo json_encode(['success' => true, 'message' => 'Đã bỏ cảm xúc', 'action' => 'removed']);
        } else {
            // Cập nhật sang loại cảm xúc mới
            $update_sql = "UPDATE cam_xuc_san_pham SET loai_cam_xuc = ? WHERE vay_id = ? AND nguoi_dung_id = ?";
            $update_stmt = $conn->prepare($update_sql);
            $update_stmt->bind_param("sii", $loai_cam_xuc, $vay_id, $user_id);
            $update_stmt->execute();
        ...
        // Thêm cảm xúc mới
        $insert_sql = "INSERT INTO cam_xuc_san_pham (nguoi_dung_id, vay_id, loai_cam_xuc) VALUES (?, ?, ?)";
        $insert_stmt = $conn->prepare($insert_sql);
        $insert_stmt->bind_param("iis", $user_id, $vay_id, $loai_cam_xuc);
        $insert_stmt->execute();
